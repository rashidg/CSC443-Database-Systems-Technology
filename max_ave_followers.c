...
int main(int argc, char *argv[]){
	...
	int current_id = -1, current_followers = 0, max_followers = 0;
	int unique_ids = 1;
	for (int rec=0;rec<N;rec++)
	{
		...
		// calculating maximum and the average number of followers per user
		if (current_id == -1) current_id = next.uid2;
		if (next.uid2 == current_id)
			current_followers++;
		else
		{
			unique_ids++;
	...
	// check max again for the last uid2
	if (current_followers > max_followers)
		max_followers = current_followers;

	printf("Maximum number of followers: %d\n", max_followers);
	printf("Average number of followers: %.3lf\n", N / (unique_ids * 1.0));
